...
func (q *Question) read(typ datautil.Type) (iface interface{}, err error) {
	var hadError bool
	line, err := q.newLine()
	if err != nil {
		return nil, err
...
func (q *Question) ReadBool() (bool, error) {
	q.isBool = true
	iface, err := q.read(datautil.Bool)
	if err != nil {
		return false, err
	}
...
func (q *Question) ReadInt64() (int64, error) {
	iface, err := q.read(datautil.Int64)
	if err != nil {
		return 0, err
	}
...
func (q *Question) ReadUint64() (uint64, error) {
	iface, err := q.read(datautil.Uint64)
	if err != nil {
		return 0, err
	}
...
func (q *Question) ReadFloat64() (float64, error) {
	iface, err := q.read(datautil.Float64)
	if err != nil {
		return 0, err
	}
...
func (q *Question) ReadString() (string, error) {
	iface, err := q.read(datautil.String)
	if err != nil {
		return "", err
	}
...
func (q *Question) readChoice(typ datautil.Type, choices interface{}) (iface interface{}, err error) {
...
func (q *Question) ChoiceInt(choices []int) (int, error) {
	value, err := q.readChoice(datautil.Int, choices)
	if err != nil {
		return 0, err
	}
...
func (q *Question) ChoiceFloat64(choices []float64) (float64, error) {
	value, err := q.readChoice(datautil.Float64, choices)
	if err != nil {
		return 0, err
	}
...
func (q *Question) ChoiceString(choices []string) (string, error) {
	value, err := q.readChoice(datautil.String, choices)
	if err != nil {
		return "", err
	}
